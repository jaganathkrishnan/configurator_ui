...
import '../../styling/WorkflowForm.css';
class WorkflowForm extends Component {
  ...
  fetchWorkflow = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const { data } = result;
        const workflowObj = new Workflow(data.workflow, data.status_tree, data.workflow_variables, data.workflow_level_actions);
        const chatbots = data.chat_bots.map((chatBotRecord) => new Chatbot(chatBotRecord));
        this.setState({
          chatbots,
          workflowObj,
        });
